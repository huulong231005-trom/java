...
public class SinhVienRepository {
    ...
    public void update(SinhVien sv) {
        ...
        if (old != null) { old.setMaSinhVien(sv.getMaSinhVien()); old.setHoTen(sv.getHoTen()); old.setEmail(sv.getEmail()); old.setLop(sv.getLop()); }
    }
    ...
    public List<SinhVien> search(String key) {
        ...
        return data.stream().filter(x -> x.getHoTen().toLowerCase().contains(k) || x.getLop().toLowerCase().contains(k)).collect(Collectors.toList());
    }
}